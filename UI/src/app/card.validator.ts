...
export function cardValidator(method: string, getDetectedFn: () => ICardType): ValidatorFn {
  ...
  return (control: AbstractControl): { [key: string]: any } => {
    ...
    const acceptedType = requiredMethodTypes[method];
    const providedType = getDetectedFn();

    return cardTypeValidator(acceptedType, providedType)
      || lengthValidator(cardNumber, providedType)
